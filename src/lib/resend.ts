...
export async function dispatchOtpEmail(
  ...
  try {
    ...
    const codeToSend = code || record.pendingCode;
    if (!codeToSend) return false;
    ...
    // Clear plaintext code from DB if present
    try {
      await OtpChallenge.updateMany({ phoneKey }, { $set: { pendingCode: "" } });
    } catch {
      // ignore
    }
